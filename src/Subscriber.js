 ...
 */
function Subscription(subscriber, request, state, expires) {
  ...
  */
  receiveRequest: function(request, initial) {
    ...
    if (!initial && !this.subscriber.matchEvent(request)) {
      if(this.ua.isDebug()) {
        console.warn(ExSIP_C.LOG_SUBSCRIBER,'NOTIFY request does not match event');
      }
      return;
    }

    request.reply(200, ExSIP_C.REASON_200, [
    ...
    switch (subscription_state.state) {
      ...
      case 'terminated':
        if (subscription_state.reason) {
          if(this.ua.isDebug()) {
            console.log(ExSIP_C.LOG_SUBSCRIBER,'terminating subscription with reason '+ subscription_state.reason);
          }
        }
        this.close();
        this.subscriber.receiveInfo(request);
  ...
  subscribe: function() {
    ...
    var request_sender = new RequestSender(this, subscription.subscriber.ua);
    ...
    this.receiveResponse = function(response) {
            ...
            if (expires && expires <= subscription.expires) {
              ...
              if (!expires) {
                if(subscription.ua.isDebug()) {
                  console.warn(ExSIP_C.LOG_SUBSCRIBER,'Expires header missing in a 200-class response to SUBSCRIBE');
                }
                subscription.subscriber.onFailure(null, ExSIP_C.EXPIRES_HEADER_MISSING);
              } else {
                if(subscription.ua.isDebug()) {
                  console.warn(ExSIP_C.LOG_SUBSCRIBER,'Expires header in a 200-class response to SUBSCRIBE with a higher value than the indicated in the request');
                }
                subscription.subscriber.onFailure(null, ExSIP_C.INVALID_EXPIRES_HEADER);
              }
            }
            break;
          default:
      ...
      this.onRequestTimeout = function() {
        subscription.subscriber.onFailure(null, ExSIP_C.causes.REQUEST_TIMEOUT);
      };

      this.onTransportError = function() {
        subscription.subscriber.onFailure(null, ExSIP_C.causes.CONNECTION_ERROR);
      };
...
Subscriber.prototype = {
  ...
  */
  close: function() {
    ...
    if (this.state !== 'terminated') {
      if(this.ua.isDebug()) {
        console.log(ExSIP_C.LOG_SUBSCRIBER,'terminating Subscriber');
      }
  ...
  */
  ...
  createSubscribeRequest: function(dialog, params) {
    ...
    var request = new SIPMessage.OutgoingRequest(ExSIP_C.SUBSCRIBE, this.ua.configuration.registrar_server, this.ua,
      params, extraHeaders);
  ...
  subscribe: function() {
    ...
    if (['notify_wait', 'pending', 'active', 'terminated'].indexOf(this.state) !== -1) {
      console.error(ExSIP_C.LOG_SUBSCRIBER,'subscription is already on');
      return;
    }

    subscriber = this;
    from_tag = Utils.newTag();

    this.request = subscriber.createSubscribeRequest(null,{from_tag:from_tag});
    var request_sender = new RequestSender(this, subscriber.ua);
    ...
    this.receiveResponse = function(response) {
        ...
        case /^2[0-9]{2}$/.test(response.status_code):
          ...
          if (expires && expires <= subscriber.expires) {
            ...
            if (!expires) {
              ...
              subscriber.onFailure(null, ExSIP_C.EXPIRES_HEADER_MISSING);
            } else {
              if(self.ua.isDebug()) {
                console.warn(ExSIP_C.LOG_SUBSCRIBER,'Expires header in a 200-class response to SUBSCRIBE with a higher value than the indicated in the request');
              }
              subscriber.onFailure(null, ExSIP_C.INVALID_EXPIRES_HEADER);
            }
          }
          break;
        default:
      ...
      this.onRequestTimeout = function() {
        subscriber.onFailure(null, ExSIP_C.causes.REQUEST_TIMEOUT);
      };

      this.onTransportError = function() {
        subscriber.onFailure(null, ExSIP_C.causes.CONNECTION_ERROR);
      };
  ...
  receiveRequest: function(request) {
    ...
    switch (subscription_state.state) {
      ...
      case 'active':
        ...
        window.clearTimeout(this.N);
        new Subscription(this, request, subscription_state.state, expires);
        break;
      case 'terminated':
        if (subscription_state.reason) {
          if(this.ua.isDebug()) {
  ...
  */
  matchEvent: function(request) {
    ...
    if (!request.hasHeader('Event')) {
      if(this.ua.isDebug()) {
        console.warn(ExSIP_C.LOG_SUBSCRIBER,'missing Event header');
      }
      return false;
    }
    ...
    if (!request.hasHeader('Subscription-State')) {
      if(this.ua.isDebug()) {
        console.warn(ExSIP_C.LOG_SUBSCRIBER,'missing Subscription-State header');
      }
      return false;
    }
    ...
    if (this.event !== event) {
      if(this.ua.isDebug()) {
        console.warn(ExSIP_C.LOG_SUBSCRIBER,'event match failed');
      }
