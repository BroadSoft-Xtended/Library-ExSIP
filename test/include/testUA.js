...
module.exports = {
  ...
  createFakeUA: function(options) {
    var ua = new UA(Utils.merge_options(this.UA_CONFIGURATION, options || {}));
    return ua;
  },
  ...
  start: function(ua, options) {
    ...
    ua.transport.send = function(msg){
      self.sendMsgs.push(msg); 
      console.log("-- Transport.send() WebSocket message:\n\n" + msg + "\n");
      return true;
    };
  },
  ...
  connect: function(ua, configuration) {
    ...
    var options = this.getMediaOptions();
    var session = ua.call(configuration["destination"] || "sip:fakeUA@net", options);
    this.triggerOnIceCandidate(session);
    ua.transport.onOpen();
  },
...
//      candidate:null});
  ...
  parseMessage: function(ua, data){
    ...
    return Parser.parseMessage(dataStr, ua);
  },
  ...
  mockWebRTC: function(){
    var self = this;
    this.isIceCandidateReadyFunction = this.isIceCandidateReadyFunction || WebRTC.RTCPeerConnection.prototype.isIceCandidateReady;
    WebRTC.RTCPeerConnection = function(){
      ...
      return {
        ...
        createOffer: function(success, failure, options){
          ...
          self.createOffer(options);
          this.signalingState = 'have-local-offer';
          success(self.createDescription(this.createDescriptionOptions()));
        },
        createAnswer: function(success, failure){
          ...
          if(!this.remoteDescription) {
            ...
            var description = self.createDescription(this.createDescriptionOptions({type: "answer"}));
            success(description);
          }
        },
        createDTMFSender: function(local_audio_track){
          console.log("-- RTCPeerConnection.createDTMFSender()");
          return new WebRTC.RTCDTMFSender();
        },
        setLocalDescription: function(description, success, failure){
          console.log("-- RTCPeerConnection.setLocalDescription() : "+Utils.toString(description));
          this.localDescription = description; self.setLocalDescription(description);
          if(success){
            success();
          }
        },
        ...
        isIceCandidateReady: function(candidate) {
          return self.isIceCandidateReadyFunction(candidate);
        }
      }
    };
    WebRTC.getUserMedia = function(constraints, success, failure){
        console.log('-- getUserMedia ');
        success(new WebRTC.MediaStream());
    };
    WebRTC.RTCDTMFSender = function(){
    ...
    WebRTC.MediaStreamTrack = function(){
        console.log('-- MediaStreamTrack.new() ');
    };
    WebRTC.MediaStream = function(){
      console.log('-- MediaStream.new()');
      return {
        stop: function(){this.ended = true; console.log("-- MediaStream.stop()")},
        getAudioTracks: function(){console.log("-- MediaStream.getAudioTracks()"); return [new WebRTC.MediaStreamTrack()];}
      };
    }
//    UA.prototype.recoverTransport = function(){console.log("--recoverTransport");}
  ...
  ringingResponse: function(ua, options) {
    ...
    var sip = "SIP/2.0 "+(options.status_code || "180 Ringing")+"\r\n"+
      ...
      "To: <sip:fakeUA@net>;tag=8c9b3674\r\n"+
      ...
      "\r\n"+
      "\r\n";
    return this.createSIPMessage(ua, sip);
  },
  ...
  sipResponseMessage: function(options, sdp) {
    var sip = "SIP/2.0 "+(options["status_code"] || "200 OK")+"\r\n"+
      ...
      "\r\n"+
      sdp;
    return this.createSIPMessage(ua, sip, options);
  },

  sipRequestMessage: function(options, sdp) {
    var sip = (options["method"] || "INVITE")+" sip:fakeUA@net SIP/2.0\r\n"+
  ...
  createDescription: function(options){
    options = options || {};
    return new WebRTC.RTCSessionDescription({type: (options["type"] || 'offer'), sdp: this.createSdp(options)});
  },
  ...
  createSdp: function(options){
    ...
    var sdp = "v=0\r\n"+
      ...
      (options["withoutAudio"] ? "" : "m=audio "+(options["audioPort"] || "16550")+" RTP/AVP "+(options["audioCodecs"] || "9 126")+"\r\n"+
        ...
        (options["withoutAudioMode"] ? "" : "a="+(options["audioMode"] || C.SENDRECV)+"\r\n")+
      ...
      (options["withoutVideo"] ? "" : "m=video "+(options["videoPort"] || "16930")+" RTP/AVP "+(options["videoCodecs"] || "99 109 34")+"\r\n"+
        ...
        (options["withoutVideoMode"] ? "" : "a="+(options["videoMode"] || C.SENDRECV)+"\r\n")+
